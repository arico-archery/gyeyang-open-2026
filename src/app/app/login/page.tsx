...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (err) {
      setError(t("이메일 또는 비밀번호가 올바르지 않습니다.", "Invalid email or password.", "邮箱或密码不正确。"));
      setLoading(false);
    } else {
      router.push("/app");
      router.refresh();
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">{t("참가선수 앱 로그인", "Athlete App Login", "选手 APP 登录")}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
          ...
          >
            {loading ? t("로그인 중...", "Signing in...", "登录中...") : t("로그인", "Sign In", "登录")}
          </button>
        </form>

        <p className="text-center text-sm text-gray-500 mt-4">
          {t("계정이 없으신가요?", "Don't have an account?", "还没有账号?")}{" "}
        ...
        <div className="text-center mt-4">
          <Link href="/" className="text-xs text-gray-400 hover:text-gray-600">
            {t("← 웹사이트로 돌아가기", "← Back to website", "← 返回网站")}
          </Link>
        </div>
      </div>
    </div>
